...
class KnotifyClient:
    def __init__(self, pseudoknot_options, hairpin_options, energy_options, sequence):
        ...
        # validate options
        self.validated_pseudoknot_options = self.validate_pseudoknot_options(
            pseudoknot_options
        )
        ...
        options_dict = {
            **self.validated_pseudoknot_options,
            **self.validated_hairpin_options,
    ...
    def validate_pseudoknot_options(self, options):
        enforce_valid_value = {
            ...
            "max_window_size": lambda v: self.get_number_within_range(
                int(v), 6, max(6, self.N)
            ),
            "min_window_size": lambda v: self.get_number_within_range(
                int(v), 6, max(6, self.N)
            ),
            "max_window_size_ratio": lambda v: self.get_number_within_range(
                float(v), 0.0, 1.0
            ),
            "min_window_size_ratio": lambda v: self.get_number_within_range(
                float(v), 0.0, 1.0
            ),
            "max_stem_allow_smaller": lambda v: self.get_number_within_range(
                int(v), 0, 10
            ),
        ...
        return {
            field: enforce_valid_value[field](options[field]) for field in common_fields
        }
    ...
    def validate_hairpin_options(self, options):
        enforce_valid_value = {
            ...
            "max_hairpins_per_loop": lambda v: self.get_number_within_range(
                int(v), 0, 2
            ),
    ...
    def get_hashed_key(self):
        """
        Returns a custom hash of the run,
        so we can later cache the resulting structure
        """
        options = dict(
            **self.validated_pseudoknot_options,
            **self.validated_hairpin_options,
        ...
        merged_items = [f"{k}:{v}" for k, v in items]
        options_string = "::".join(merged_items)
        key = f"{self.sequence}:::{options_string}"
        hashed_key = f'run:{sha1(key.encode("utf-8")).hexdigest()}'
        return hashed_key
